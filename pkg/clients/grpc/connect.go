...
func connect(cfg Config) (*grpc.ClientConn, security, error) {
	...
	if cfg.AttestedTLS {
		err := ReadManifest(cfg.Manifest, &attest.AttConfigurationSEVSNP)
		if err != nil {
...
func ReadManifest(manifestPath string, attestationConfiguration *attest.AttestationConfiguration) error {
	if manifestPath != "" {
